...
public class DataManager {
    ...
    public Collection<Position> getPositions(String devId) throws SQLException {
        Connection conn = null;
        Properties connectionProps = new Properties();
        connectionProps.put("user", "ronly");
        connectionProps.put("password", "ronly");
        conn = DriverManager.getConnection("jdbc:postgresql://pgbouncer:5432/tsdb",connectionProps);

        PreparedStatement statement = conn.prepareStatement("SELECT payload, ts_device " +
                "FROM \"data-wks-6o7yvdo8inlt\" WHERE device_id = ?");
        statement.setString(1,devId);
        String table = "";
        ResultSet set = statement.executeQuery();

        //System.out.println("CONNECTED TO zDB - QUERY EXECUTED");
        ...
        while (set.next()) {
            ...
            String [] tmp = payload.split("\"latitude\":");
            String lat = tmp[1].split(",")[0];

            tmp = payload.split("\"longitude\":");
            String lon = tmp[1].split("}")[0];
            ...
            p.setId(id);
            p.setLongitude(Float.parseFloat(lon));
            p.setLatitude(Float.parseFloat(lat));

            String date = set.getString("ts_device");
            String [] split = date.split(" ");
            String formatted_data = split[0] +"T"+split[1];

            p.setDeviceTime(Date.from(Instant.from(DateTimeFormatter.ISO_ZONED_DATE_TIME.parse(formatted_data))));
            p.setServerTime(Date.from(Instant.from(DateTimeFormatter.ISO_ZONED_DATE_TIME.parse(formatted_data))));
            p.setFixTime(Date.from(Instant.from(DateTimeFormatter.ISO_ZONED_DATE_TIME.parse(formatted_data))));
            p.setValid(true);
            p.setProtocol("osmand");
